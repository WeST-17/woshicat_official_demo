...
const UserConsent = () => {
    const { consent, setConsent, hideNotice, setHideNotice, setLoaded } = useMetaPixel();
    ...
    return (
        <>
            {(consent === null && hideNotice === false) && (
                <section className={`z-[3100] bg-white/90 p-8 fixed bottom-0 w-full h-fit flex flex-col justify-end items-center transition-all duration-500 gap-4 ${hideNotice ? 'hidden' : ''}`}>
                    <div className="w-full flex justify-center items-center">
                        <h2 className="w-full lg:w-2/3 text-sm">
                        ...
                        <div className="relative w-full lg:w-1/3 flex flex-col justify-center items-center text-base transition-all duration-500 gap-1">
                            <button 
                                ...
                                onClick={() => { setHideNotice(true); setLoaded(true); }}
                            >
                                {`close`}
                            </button>
                            ...
                            <button className="w-40 h-fit rounded-md border border-2 border-stone-400 flex justify-center items-center hover:text-red-800" onClick={() => userSubmitConsent('reject')}>
                                Reject
                            </button>
                        </div>
                    </div>

...
const Header = () => {
    ...
    return (
        <>
            <header className={`fixed absolute top-0 h-[70px] w-screen ${darkMode ? `dark-text bg-stone-700/65 hover:bg-stone-700/90` : `bg-white/60 hover:bg-white/90 light-text`} text-base p-2 z-2002 transition duration-500 flex ${!isVisible && !cartOpen ? '-translate-y-[70px]' : ''}`} id="header">
                ...
                <div className={`p-2 absolute max-lg:left-0 relative w-fit lg:mx-auto h-full flex justify-start lg:justify-center items-center transition transition-all duration-500 ease-in-out ${isActive ? "z-3001" : "z-2000"} ${darkMode ? "dark-text" : "light-text"}`} onClick={activeMenu}>
                    <Navigation />
                </div>

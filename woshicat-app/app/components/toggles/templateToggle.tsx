        ...
        <>
            {!loading && (
                <button className="relative inline-block w-10 h-5 flex justify-center items-center">
                    <input id={`switch-component-${switch_id}`} type="checkbox" className={`peer appearance-none w-full h-full rounded-full ${alwaysOn ? 'bg-stone-300 checked:bg-red-900/50 pointer-events-none' : 'bg-stone-500 checked:bg-red-900' } cursor-pointer transition-colors duration-300 `} onChange={toggleFunction} checked={state} readOnly={alwaysOn ? true : false}/>

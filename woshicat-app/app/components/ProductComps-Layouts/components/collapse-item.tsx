...
const Collapse: React.FC<CollapseProps> = ({ children, title, classProp, divider = true}) => {
    ...
    return (
        <>
            <span className={`flex w-full h-[0.5px] bg-stone-600/50 rounded-full ${divider ? "" : "hidden"}`} /> 
            <div className={`px-2 my-3 relative w-full h-fit flex flex-col justify-start items-start ${classProp}`}>
                <button 
                    onClick={toggleItem} 
                    className={`relative w-full flex justify-start transition transition-all duration-350 ${!openItem ? 'font-medium' : 'font-bold'}`}
                >

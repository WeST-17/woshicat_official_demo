    ...
    return (
        <div className={`w-full flex flex-col`}>
            <div className="w-full flex relative items-center">
                <button className={`absolute left-0 md:-left-10 h-1/2 z-[100] bg-stone-300 p-3 rounded-md opacity-35 hover:opacity-100 transition duration-300 ${length > (1) ? '' : 'hidden'} ${length <= numPerSlide ? 'lg:hidden' : ''}`} onClick={prev}>
                ...
                <div className="overflow-hidden w-full h-full">
                    <div
                        ...
                        style={{
                            ...
                            1 / (windowSize.width > 1024 ? numPerSlide : (1)) * 100
                            )}%)`,
                        }}
                        ref={childRef}
                        >
                ...
                <button className={`absolute h-1/2 right-0 md:-right-10 z-[100] bg-stone-300 rounded-md p-3 opacity-35 hover:opacity-100 transition duration-300 ${length > (1) ? '' : 'hidden'} ${length <= numPerSlide ? 'lg:hidden' : ''}`} onClick={next} >

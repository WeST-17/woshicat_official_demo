                    ...
                    {cartItems.map((item: any, i) => {
                        ...
                        <div key={`b_${i}`} className={`${styles.linkContainer} flex items-center justify-start w-full ${item.quantity <= 0 ? 'opacity-70 pointer-events-none' : ''}`}>
                            <motion.div
                                ...
                                className={`flex h-full w-full gap-2 p-1`}
                            >
                                <Link href={`https://woshicat.com/collections/${item.collection}/${item.handle}`} className={`h-full overflow-hidden rounded-lg transition transition-all duration-500 ${cartOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                                    <Image
                                        src={item.imageUrl}
                                        alt={item.handle}

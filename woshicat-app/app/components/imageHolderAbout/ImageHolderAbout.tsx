                ...
                <div className="bg-white rounded-md relative w-full h-3/4 flex max-md:flex-col justify-center items-center gap-2 max-lg:p-8 max-lg:m-2">
                    {
                        React.Children.toArray(children).map((child, index) => (
                                <>
                                <div key={index} className={`z-2005 w-1/2 h-1/4 flex justify-center items-center transition-all duration-300 text-white rounded-xs pointer-events-none ${index === current ? 'opacity-100 sway' : 'opacity-0 hidden'}`}>
                                    {child}
                                </div>
                                <Card description={about_descriptions[index]} classAdd={`max-lg:text-sm aspect-square rounded-xs transition-all duration-300 max-md:w-full lg:w-3/4 h-full md:h-3/4 pe-2 ${index === current ? 'opacity-100' : 'opacity-0 hidden'}`}/>
                                </>
                            )
                        )
                    }
